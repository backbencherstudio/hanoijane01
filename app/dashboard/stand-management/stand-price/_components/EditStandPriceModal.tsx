...
import { StandPriceData } from "./CreateStandPriceModal";
...
interface EditStandPriceModalProps {
  isOpen: boolean;
  onClose: () => void;
  standId: string;
  onUpdate?: (data: StandPriceData) => void;
}

const EditStandPriceModal = ({
  isOpen,
  onClose,
  standId,
  onUpdate,
}: EditStandPriceModalProps) => {
  const [open, setOpen] = useState(false);

  // Find the stand package by id
  const standData = standPackages.find((pkg) => pkg.id === standId);
  ...
  // Populate form when standData changes or modal opens
  useEffect(() => {
    if (isOpen && standData) {
      reset({
        title: standData.title || "",
        description: standData.description || "",
        currency: (standData as Partial<StandPriceData>).currency || "€",
        price: standData.price || 0,
        vatIncluded:
          ((standData as Partial<StandPriceData>).vatIncluded as boolean) ||
          false,
        includes: standData.includes?.length
          ? standData.includes.map((item) => ({ value: item }))
          : [{ value: "" }],
      });
    }
  }, [isOpen, standData, reset]);
  ...
  const handleFormSubmit = (data: StandPriceFormData) => {
    ...
    console.log("Updated Stand Price:", payload);
    onUpdate?.(payload);
    reset();
    onClose();
  };
...
export default EditStandPriceModal;
